    ...
    const knowledgeSubItems =
        language === 'vi'
            ...
            : [
                  ...
                  {
                      label: 'Porcelain Knowledge',
                      to: ROUTES.knowledgePorcelain,
                  },
              ];
        ...
        <header className='border-b border-slate-200 bg-white shadow-sm'>
            <div className='mx-auto w-full max-w-[1360px] px-4'>
                <div className='flex flex-wrap items-center gap-4 py-3 lg:flex-nowrap'>
                    ...
                    <form
                        className='relative w-full lg:max-w-[760px]'
                    ...
                    <div className='ml-auto flex items-center gap-4'>
                        ...
                        <div className='relative' ref={languageMenuRef}>
                            ...
                            >
                                ...
                                <span
                                    className={`text-xs text-slate-400 transition ${isLanguageMenuOpen ? 'rotate-180' : ''}`}
                                >
                                    &#9662;
                                </span>
                            </button>
                ...
                <div className='flex items-center justify-between border-t border-slate-100 pb-3 pt-2'>
                    <nav className='flex flex-wrap items-center gap-5'>
                        ...
                        >
                            ...
                            >
                                {text.aboutMenu}
                                <span
                                    className={`text-sm text-slate-400 transition ${isAboutMenuOpen ? 'rotate-180' : ''}`}
                                >
                                    &#9662;
                                </span>
                            </button>
                            ...
                            <div
                                className={`absolute left-0 top-full z-20 w-[380px] rounded-2xl border border-slate-200 bg-white p-2 shadow-lg transition ${isAboutMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}
                            >
                                {aboutSubItems.map((item) => (
                                    <NavLink
                                        key={item.to}
                                        className={({ isActive }) =>
                                            `block rounded-xl px-4 py-3 text-base font-medium transition ${isActive ? 'bg-slate-100 text-slate-900' : 'text-slate-700 hover:bg-blue-600 hover:text-white'}`
                                        }
                                        onClick={() =>
                                            setIsAboutMenuOpen(false)
                                        }
                        ...
                        >
                            ...
                            >
                                {text.serviceMenu}
                                <span
                                    className={`text-sm text-slate-400 transition ${isServiceMenuOpen ? 'rotate-180' : ''}`}
                                >
                                    &#9662;
                                </span>
                            </button>
                        ...
                        <div
                            ...
                            onMouseLeave={() => setIsPriceMenuOpen(false)}
                            ref={priceMenuRef}
                        >
                            <button
                                className={`inline-flex items-center gap-1 border-b-2 px-0.5 py-2 text-base font-semibold transition ${
                                    isPriceActive || isPriceMenuOpen
                                        ? 'border-slate-900 text-slate-900'
                                        : 'border-transparent text-slate-700 hover:border-slate-300 hover:text-slate-900'
                                }`}
                                onClick={() =>
                            ...
                            >
                                {text.priceMenu}
                                <span
                                    className={`text-sm text-slate-400 transition ${isPriceMenuOpen ? 'rotate-180' : ''}`}
                                >
                                    &#9662;
                                </span>
                            </button>
                            ...
                            <div
                                className={`absolute left-0 top-full z-20 w-[380px] rounded-2xl border border-slate-200 bg-white p-2 shadow-lg transition ${isPriceMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}
                            >
                        ...
                        >
                            <button
                                className={`inline-flex items-center gap-1 border-b-2 px-0.5 py-2 text-base font-semibold transition ${
                                    isKnowledgeActive || isKnowledgeMenuOpen
                                        ? 'border-slate-900 text-slate-900'
                                        : 'border-transparent text-slate-700 hover:border-slate-300 hover:text-slate-900'
                                }`}
                                onClick={() =>
                                    setIsKnowledgeMenuOpen((prev) => !prev)
                                }
                                type='button'
                            >
                                {text.knowledgeMenu}
                                <span
                                    className={`text-sm text-slate-400 transition ${isKnowledgeMenuOpen ? 'rotate-180' : ''}`}
                                >
                                    &#9662;
                                </span>
                            </button>
                            ...
                            <div
                                className={`absolute left-0 top-full z-20 w-[380px] rounded-2xl border border-slate-200 bg-white p-2 shadow-lg transition ${isKnowledgeMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}
                            >
                        ...
                        {topMenuItems.map((item) => (
                            <button
                                key={item.label}
                                className={`inline-flex items-center gap-1 border-b-2 px-0.5 py-2 text-base font-semibold transition ${item.isActive ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-700 hover:border-slate-300 hover:text-slate-900'}`}
                                type='button'
                            >
                                {item.isHot ? (
                    ...
                    <div className='flex items-center gap-2'>
                        <HeaderAuthActions
                            loginLabel={text.loginButton}
                            registerLabel={text.registerButton}
                        />
                        <button
