...
export default function TopServicesGroupedChart() {
  ...
  const { data, isFetching, isError, error } = useGetTopServicesByAppointmentsGroupedQuery(params);
  ...
  const { chartData, periods } = useMemo(() => {
    ...
    for (const row of items) {
      ...
      if (!serviceIdToName.has(row.serviceId)) {
        serviceIdToName.set(row.serviceId, row.serviceName);
      }
      if (!cell.has(row.period)) {
        cell.set(row.period, new Map());
      }
      const m = cell.get(row.period)!;
      m.set(row.serviceId, (m.get(row.serviceId) ?? 0) + row.appointmentCount);
    }

    const sortedPeriods = [...periodSet].sort();
    const services = [...serviceIdToName.entries()].sort((a, b) =>
    ...
    const datasets = services.map(([serviceId, serviceName], idx) => ({
      ...
      borderColor: SERVICE_BAR_COLORS[idx % SERVICE_BAR_COLORS.length].replace('0.75', '1'),
      borderWidth: 1,
      borderRadius: 2,
      stack: 'services',
    }));
